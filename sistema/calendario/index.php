...
if (isset($_POST['from'])) 
{
    ...
    if ($_POST['from']!="" AND $_POST['to']!="") 
    {
        ...
        $inicio = _formatear($_POST['hora_inicio']);
        // y la formateamos con la funcion _formatear

        $final  = _formatear($_POST['hora_fin']);
        ...
        $titulo = $_POST['nombre_servicio'];

        // y con la funcion evaluar
        $body   = $_POST['duracion'];

        // reemplazamos los caracteres no permitidos
        $clase  = $_POST['nombre_usuario'];

        // insertamos el evento
        $query="INSERT INTO control_citas VALUES(null,'$titulo','$body','','$clase','$inicio','$final','$inicio_normal','$final_normal')";

        // Ejecutamos nuestra sentencia sql
        $conexion->query($query); 

        // Obtenemos el ultimo id insetado
        $im=$conexion->query("SELECT MAX(id) AS id FROM control_citas");
        $row = $im->fetch_row();  
        $id = trim($row[0]);
        ...
        $query="UPDATE control_citas SET url = '$link' WHERE id = $id";
