...
(function($) {
	...
	Calendar.prototype._calculate_hour_minutes = function(data) {
		...
		$.each(data.events, function(k, e) {
			...
			s.setMinutes(s.getMinutes()+360).toString().formatNum(2);
			f.setMinutes(f.getMinutes()+360).toString().formatNum(2);
	...
	Calendar.prototype._loadTemplate = function(name) {
		...
		var self = this;
		$.ajax({
			url:      this.options.tmpl_path + name + '.php',
			dataType: 'html',
			type:     'GET',
			async:    false,
			cache:    this.options.tmpl_cache
		}).done(function(php) {
				self.options.templates[name] = _.template(php);
			});
	};
				...
				modal
					.on('show.bs.modal', function() {
						...
						switch(self.options.modal_type) {
							...
							case "ajax":
								$.ajax({url: url, dataType: "php", async: false, success: function(data) {
