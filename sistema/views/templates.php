...
class templates{
    ...
    public function genera_campos($input, $campo, $valor,$arreglo, $clase_elimina,$link){
        ...
        if(in_array($accion,$vistas)) {
            ...
            elseif ($input['tipo'] == 'checkboxDia') {
                $html = $html . $directiva->checkboxDia($valor, $cols, $campo);
            }
            elseif ($input['tipo'] == 'checkbox') {
                $html = $html . $directiva->checkbox($valor, $cols, $campo);
            }
            elseif ($input['tipo'] == 'editortexto') {
                $html = $html . $directiva->editortexto($valor, $cols, $campo);
            }
            ...
            elseif ($input['tipo'] == 'hora') {
                $html = $html . $directiva->hora($valor, $cols, $campo);
            }
    ...
    public function obten_elementos_body($tabla, $registro){
        ...
        foreach($this->campos as $campo){
            ...
            $tipo = false;
            if($campo == 'video_inicio_archivo'){
                $tipo = 'video';
            }
            elseif($campo == 'grupo_servicios_archivo'){
                $tipo = 'imagen';
            }
            elseif ($campo == 'servicios_archivo'){
                $tipo = 'imagen';
            }
            elseif ($campo == 'imagen_servicio_archivo'){
                $tipo = 'imagen';
            }
            ...
            elseif ($campo == 'slider_inicio_archivo'){
                $tipo = 'imagen';
            }
            elseif ($campo == 'instalaciones_archivo'){
                $tipo = 'imagen';
            }
             elseif ($campo == 'cabina_archivo'){
                $tipo = 'imagen';
            }
            ...
            if($campo == $tabla.'_status'){
                ...
                if(sizeof($etiqueta) <= 2){
                      $html = $html.$directiva->span_etiqueta($etiqueta[1], $registro[$campo],$tipo);

                }else if(sizeof($etiqueta) > 2){
                      $html = $html.$directiva->span_etiqueta($etiqueta[2], $registro[$campo],$tipo);
