...
class Modelos{
    ...
    public function obten_servicios_citas($tabla){
        $consulta = "SELECT s.id,s.nombre,s.descripcion_corta,s.descripcion,s.caracteristicas,s.que_incluye,s.archivo,s.duracion,s.precio,s.status,s.subgrupo_servicios_id, ss.nombre, gs.nombre from((servicios as s left outer join subgrupo_servicios as ss on subgrupo_servicios_id=ss.id) left outer join grupo_servicios as gs on ss.grupo_servicios_id=gs.id)";
        $where = " WHERE $tabla.status=1 ";
        $consulta = $consulta.$where;
        $result = $this->ejecuta_consulta($consulta);
        return $result;
